(function () {
  ...
  function rememberStorageIssue(action, key, error) {
    lastStorageIssue = {
      action: action,
      key: key,
      message: error && error.message ? error.message : "armazenamento local indisponível",
      at: new Date().toISOString()
    };
    console.warn("Plenna: armazenamento local indisponível", action, key, error);
    try {
      window.dispatchEvent(new CustomEvent("plenna:storage-error", { detail: lastStorageIssue }));
    } catch (dispatchError) {
      console.warn("Plenna: não foi possível avisar sobre armazenamento local", dispatchError);
    }
  }
  ...
  function clearStorageIssue() {
    lastStorageIssue = null;
  }

  function read(key, fallback) {
    var namespacedKey = fullKey(key);
    try {
      var raw = localStorage.getItem(namespacedKey);
      if (raw) return JSON.parse(raw);
      if (Object.prototype.hasOwnProperty.call(memoryStore, namespacedKey)) return clone(memoryStore[namespacedKey]);
      return fallback;
    } catch (error) {
      rememberStorageIssue("ler", key, error);
      if (Object.prototype.hasOwnProperty.call(memoryStore, namespacedKey)) return clone(memoryStore[namespacedKey]);
      return fallback;
    }
  }
  ...
  function write(key, value) {
    var namespacedKey = fullKey(key);
    memoryStore[namespacedKey] = clone(value);
    try {
      localStorage.setItem(namespacedKey, JSON.stringify(value));
      clearStorageIssue();
    } catch (error) {
      rememberStorageIssue("salvar", key, error);
    }
    return value;
  }
  ...
  function removeKey(key) {
    var namespacedKey = fullKey(key);
    delete memoryStore[namespacedKey];
    try {
      localStorage.removeItem(namespacedKey);
      clearStorageIssue();
    } catch (error) {
      rememberStorageIssue("remover", key, error);
    }
  }
  ...
  function buildDemoData() {
    ...
    var tasks = [
      ...
      },
      {
        ...
        descricao: "Tarefa concluída para mostrar seu progresso.",
        prioridade: "baixa",
        esforco: "baixo",
        tempoEstimado: "10 min",
        impactoEmocional: "leve",
    ...
    for (var todayIndex = 1; todayIndex <= 2; todayIndex += 1) {
      tasks.push({
        ...
        descricao: "Tarefa inicial para manter o dia com uma prioridade visível.",
        prioridade: "media",
        esforco: "baixo",
    ...
    for (var doneIndex = 1; doneIndex <= 21; doneIndex += 1) {
      tasks.push({
        ...
        descricao: "Histórico de exemplo para seu progresso e aba de concluídas.",
        prioridade: "baixa",
        esforco: "baixo",
        tempoEstimado: "10 min",
        impactoEmocional: "leve",
    ...
    for (var checkinIndex = 4; checkinIndex <= 11; checkinIndex += 1) {
      checkins.push({
        ...
        nota: "Registro inicial para sequência emocional."
      });
    }
    ...
    var reminders = [
      {
        ...
        title: "Check-in",
        body: "08:30 em dias úteis. Se passar do horário, o aviso volta mais leve à tarde.",
        time: "08:30",
        active: true,
      ...
      },
      {
        ...
        body: "Depois de foco longo. Em energia baixa, aparece antes.",
        time: "15:30",
        active: true,
      ...
      },
      {
        ...
        title: "Hábitos",
        body: "Hidratação perto das 15h, com tom leve quando a meta já foi feita.",
        time: "15:00",
        active: true,
      ...
      },
      {
        ...
        body: "20:00 às 07:00. Reduz avisos e protege descanso.",
        time: "20:00",
        active: true,
    ...
    var exportContent = JSON.stringify({
      ...
      inclui: ["checkins", "tasks", "habits", "focusSessions", "entries"],
      dados: {
        checkins: checkins,
        tasks: tasks,
        habits: habits,
      ...
      exports: [
        {
          ...
          tipo: "relatório do Plenna",
          nomeArquivo: "plenna-relatorio-" + today + ".json",
  ...
  function clearAppData(options) {
    ...
    ["focusDraft", "microPauseDraft", "planningAdjustments", "selectedCalendarDay"].forEach(function (key) {
      removeKey(key);
    });
    if (options && options.allowAutoSeed) removeKey(DEMO_STATE_KEY);
